...
class BertFPNGated(nn.Module):
    """
    BERT + Gated FPN for text classification
    Uses learnable gates to weight contributions from different layers
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        layer_indices=(4, 8, 12),  # 使用第4、8、12层
    ):
        super().__init__()
        ...
        self.classifier = nn.Sequential(
            nn.Linear(fpn_dim * 3, classifier_hidden),  # 拼接三层特征
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def _init_added_layers(self):
        ...
        added_layers = [
            ...
            self.gate8[0], self.gate8[2],
            self.gate4[0], self.gate4[2],
    ...
    def forward(
        ...
        # Gated FPN top-down fusion
        p12 = c12
        
        # Fusion for p8
        x8 = c8
        y8 = self.fpn_proj12_to_8(p12)
        gate_input8 = torch.cat([x8, y8], dim=-1)  # [B, L, 512]
        g8 = self.gate8(gate_input8)  # [B, L, 1]
        p8 = self.norm8(g8 * x8 + (1 - g8) * y8)
        
        # Fusion for p4
        x4 = c4
        y4 = self.fpn_proj8_to_4(p8)
        gate_input4 = torch.cat([x4, y4], dim=-1)  # [B, L, 512]
        g4 = self.gate4(gate_input4)  # [B, L, 1]
        p4 = self.norm4(g4 * x4 + (1 - g4) * y4)
