...
class BertFPNTwoLayer(nn.Module):
    """
    BERT + Two-Layer FPN for text classification
    Uses only layers 8 and 12 for fusion
    """
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(fpn_dim * 2, classifier_hidden),  # 拼接两层特征
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def _init_added_layers(self):
        ...
        added_layers = [
            self.proj8, self.proj12,
            self.fpn_proj12_to_8,
    ...
    def forward(
        ...
        # Concat & classify
        v = torch.cat([v8, v12], dim=-1)          # [B, 512]
