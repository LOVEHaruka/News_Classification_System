...
FIGURES_DIR = os.path.join(BASE_DIR, 'figures_fpn_data1')
...
print(f"Categories: {AG_NEWS_LABELS}")
...
def tokenize_data(train_df, val_df, test_df, tokenizer, max_length):
    ...
    # 转换 Class Index 为 0-based (从 1-4 转换为 0-3)
    train_df['label'] = train_df['Class Index'].astype(int) - 1
    val_df['label'] = val_df['Class Index'].astype(int) - 1
...
def train_and_evaluate(model_name, tokenizer, train_dataset, val_dataset, test_dataset, epochs=50, **model_kwargs):
    ...
    best_val_accuracy = 0
    best_model_dir = os.path.join(BASE_DIR, f'{model_name}_data1_model')
    ...
    with open(result_file, 'w', encoding='utf-8') as f:
        f.write("="*80 + "\n")
        f.write(f"{model_name} Model Training Results (Data1 - AG News)\n")
        f.write("="*80 + "\n\n")
...
def main():
    ...
    # 定义所有可用的FPN模型
    fpn_models = {
        '1': ('bert_fpn', {'dropout': 0.3}),
        '2': ('bert_fpn_two_layer', {'dropout': 0.3}),
        '3': ('bert_fpn_gated', {'dropout': 0.3}),
        '4': ('all', {})  # 训练所有模型
    }
    
    # 显示模型选择菜单
    print("\n" + "="*80)
    print("Available FPN Models for Data1 (AG News - 4 Classes)")
    print("="*80)
    print("1. BERT + FPN (Single Layer)")
    print("2. BERT + FPN (Two Layer)")
    print("3. BERT + FPN (Gated)")
    print("4. Train All Models")
    print("="*80)
    
    # 获取用户选择
    choice = input("\nPlease select a model to train (1-4): ").strip()
    
    # 根据选择确定要训练的模型
    if choice == '4':
        models_to_train = [fpn_models['1'], fpn_models['2'], fpn_models['3']]
    elif choice in fpn_models and choice != '4':
        models_to_train = [fpn_models[choice]]
    else:
    ...
    if len(models_to_train) > 1:
        print(f"\n{'='*80}")
        print("Model Comparison - Data1 (AG News)")
        print(f"{'='*80}")
